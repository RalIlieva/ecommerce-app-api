...
#     def test_complete_checkout_flow(self, mock_payment_intent_retrieve, mock_payment_intent_create):
...
#         start_response = self.client.post(start_checkout_url, format='json', data={'shipping_address': '456 Elm St'})
#
#         self.assertEqual(start_response.status_code, status.HTTP_201_CREATED)
#         self.assertIn('payment_secret', start_response.data)
#         self.assertEqual(start_response.data['payment_secret'], 'complete_flow_secret')
#
...
#         complete_checkout_url = reverse('checkout:complete-checkout', kwargs={'checkout_session_uuid': checkout_session.uuid})
#         complete_response = self.client.post(complete_checkout_url, format='json', data={'payment_status': 'SUCCESS'})
#
#         self.assertEqual(complete_response.status_code, status.HTTP_200_OK)
#         self.assertEqual(complete_response.data['detail'], "Checkout completed successfully.")
#         self.assertEqual(complete_response.data['order_id'], str(checkout_session.payment.order.uuid))
#
...
#         mock_payment_intent_retrieve.assert_called_once_with('pi_complete_flow')
