...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        User = get_user_model()
        duplicate_uuids = User.objects.values('uuid').annotate(uuid_count=Count('uuid')).filter(uuid_count__gt=1)

        if not duplicate_uuids.exists():
            self.stdout.write(self.style.SUCCESS("No duplicate UUIDs found in User model."))
            return
        ...
        for entry in duplicate_uuids:
            ...
            users_with_duplicate_uuid = User.objects.filter(uuid=duplicated_uuid)
            # Retain the first user, assign new UUIDs to the rest
            for user in users_with_duplicate_uuid[1:]:
                new_uuid = uuid.uuid4()
                user.uuid = new_uuid
                user.save()
                self.stdout.write(f"Assigned new UUID {new_uuid} to user {user.email}")

        self.stdout.write(self.style.SUCCESS("All duplicate UUIDs have been resolved."))
